...
class TfaLoginForm extends UserLoginForm {
  ...
  public function loginWithTfa(FormStateInterface $form_state) {
    ...
    if ($this->tfaContext->pluginAllowsLogin()) {
      ...
      $enabled_send_plugins = $this->tfaSettings->get('send_plugins');

      //$this->tfaSendPlugins = $this->tfaSendManager->getPlugins(['uid' => $user->id()]);
      foreach ($enabled_send_plugins as $enabled_send_plugin){
        $instance = $this->tfaSendManager->createInstance($enabled_send_plugin, ['uid' => $user->id(), 'user' => $user, 'settings' => $this->tfaSettings]);
        $instance->begin();
      }
